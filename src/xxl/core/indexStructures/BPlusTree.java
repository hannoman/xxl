...
/**
 * This class implements a B+Tree. For a detailed discussion see Douglas Comer:
 * "The Ubiquitous B-Tree", ACM Comput. Surv. 11(2), 121-137, 1979.
 */
public class BPlusTree extends Tree {
	...
	/**
	 * This is the maximal capacity of the tree's leaf nodes, i.e. the maximal
	 * number of entries which a node can contain. Its value is the quotient of
	 * the given block size and the maximal entry size.
	 * 
	 * @see #BLOCK_SIZE
	 */
	protected int B_LeafNode;
	/**
	 * This is the minimal capacity of the tree's index nodes, i.e. the minimal
	 * number of entries which a node can contain. Its value is:
	...
	/**
	 * This is the minimal capacity of the tree's leaf nodes, i.e. the minimal
	 * number of entries which a node can contain. Its value is:
	...
	/**
	 * Creates a new <tt>BPlusTree</tt>. With a default setting of duplicates =
	 * false
	 * 
	 * @param blockSize
	 ...
	 * the default value 0.5 (i.e. 50%). and a default setting of duplicates =
	 * false
	 * 
	 * @param blockSize
	 *            the block size of the underlying storage.
	...
	/**
	 * Creates a new <tt>BPlusTree</tt>. with the option to set duplicates
	 * 
	 * @param blockSize
	 *            the block size of the underlying storage
	 * @param minCapacityRatio
	 ...
	 */
	protected BPlusTree initialize(Function getKey, MeasuredConverter keyConverter, MeasuredConverter dataConverter, Function createSeparator, Function createKeyRange, Function getSplitMinRatio, Function getSplitMaxRatio) {
		this.getKey = getKey;
		...
		return (BPlusTree) super.initialize((IndexEntry) null, (Descriptor) null, getDescriptor, getContainer, determineContainer, underflows, overflows, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 * @return the initialized <tt>BPlusTree</tt> itself
	 */
	public BPlusTree initialize(Function getKey, Function getContainer, Function determineContainer, MeasuredConverter keyConverter, MeasuredConverter dataConverter, Function createSeparator, Function createKeyRange, Function getSplitMinRatio, Function getSplitMaxRatio) {
		this.getContainer = getContainer;
		this.determineContainer = determineContainer;
		return initialize(getKey, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 * {@link #initialize (Function getKey, Container container, MeasuredConverter keyConverter, MeasuredConverter dataConverter, Function createSeparator, Function createKeyRange, Function getSplitMinRatio, Function getSplitMaxRatio)}
	 * .
	 * 
	 * @param getKey
	 *            the <tt>Function</tt> to get the key of a data object
	 ...
	 */
	public BPlusTree initialize(Function getKey, final Container container, MeasuredConverter keyConverter, MeasuredConverter dataConverter, Function createSeparator, Function createKeyRange, Function getSplitMinRatio, Function getSplitMaxRatio) {
		Function getContainer = new Constant(container);
		Function determineContainer = new Constant(container);
		return initialize(getKey, getContainer, determineContainer, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 * </pre></code> NOTE: This method is used in the case that the tree has
	 * only one container. For multidisk storage the user has to use the method:
	 ...
	 * @return the initialized <tt>BPlusTree</tt> itself
	 * 
	 * @see #initialize (Function, Function, Function, MeasuredConverter,
	 *      MeasuredConverter, Function, Function)
	 * @see #initialize (Function, Container, MeasuredConverter,
	 *      MeasuredConverter, Function, Function, Function, Function)
	 */
	public BPlusTree initialize(final Function getKey, final Container container, MeasuredConverter keyConverter, MeasuredConverter dataConverter, final Function createSeparator, final Function createKeyRange) {
		Function getSplitMinRatio = new Constant(this.minCapacityRatio);
		Function getSplitMaxRatio = new Constant(1.0);
		return initialize(getKey, container, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 *      MeasuredConverter, Function, Function)
	 */
	public BPlusTree initialize(Function getKey, Function getContainer, Function determineContainer, MeasuredConverter keyConverter, MeasuredConverter dataConverter, final Function createSeparator, final Function createKeyRange) {
		Function getSplitMinRatio = new Constant(this.minCapacityRatio);
		Function getSplitMaxRatio = new Constant(1.0);
		return initialize(getKey, getContainer, determineContainer, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	}
	...
	/**
	 * Deprecated because only used from {@link
	 * xxl.core.indexStructures.BPlusTree.initialize(Function,
	 * MeasuredConverter, MeasuredConverter, Function, Function, Function,
	 * Function)} Moving functionality there.
	 * 
	 * Convenience initialization function. Taking {@link Tree#getContainer} and
	 * {@link Tree#determineContainer} from (already set?) fields.
	 * 
	 ...
	 */
	@Deprecated
	protected BPlusTree initialize(Function getDescriptor, Predicate underflows, Predicate overflows, Function getSplitMinRatio, Function getSplitMaxRatio) {
		return initialize(getDescriptor, this.getContainer, this.determineContainer, underflows, overflows, getSplitMinRatio, getSplitMaxRatio);
	}

	/**
	 * Deprecated, as it is only used by {@link
	 * xxl.core.indexStructures.BPlusTree.initialize(Function,
	 * MeasuredConverter, MeasuredConverter, Function, Function, Function,
	 * Function)}. Moving functionality there.
	 * 
	 * Actual initialization. Initializes the <tt>BPlusTree</tt> and sets the
	 * member fields <tt>rootEntry</tt> and <tt>rootDescriptor</tt> to null.
	 * 
	 * @param getDescriptor
	 *            the new {@link xxl.core.indexStructures.Tree#getDescriptor}
	 * @param getContainer
	 ...
	 */
	@Deprecated
	protected BPlusTree initialize(Function getDescriptor, Function getContainer, Function determineContainer, Predicate underflows, Predicate overflows, Function getSplitMinRatio, Function getSplitMaxRatio) {
		return (BPlusTree) super.initialize((IndexEntry) null, (Descriptor) null, getDescriptor, getContainer, determineContainer, underflows, overflows, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 * @return the initialized <tt>BPlusTree</tt> itself
	 */
	public BPlusTree initialize(IndexEntry rootEntry, Descriptor rootDescriptor, final Function getKey, final Container container, MeasuredConverter keyConverter, MeasuredConverter dataConverter, final Function createSeparator, final Function createKeyRange) {
		Function getSplitMinRatio = new Constant(this.minCapacityRatio);
		Function getSplitMaxRatio = new Constant(1.0);
		return initialize(rootEntry, rootDescriptor, getKey, container, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 * @return the initialized <tt>BPlusTree</tt> itself
	 */
	public BPlusTree initialize(IndexEntry rootEntry, Descriptor rootDescriptor, Function getKey, final Container container, MeasuredConverter keyConverter, MeasuredConverter dataConverter, Function createSeparator, Function createKeyRange, Function getSplitMinRatio, Function getSplitMaxRatio) {
		Function getContainer = new Constant(container);
		Function determineContainer = new Constant(container);
		return initialize(rootEntry, rootDescriptor, getKey, getContainer, determineContainer, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	}
	 ...
	 * @return the initialized <tt>BPlusTree</tt> itself
	 */
	public BPlusTree initialize(IndexEntry rootEntry, Descriptor rootDescriptor, Function getKey, Function getContainer, Function determineContainer, MeasuredConverter keyConverter, MeasuredConverter dataConverter, Function createSeparator, Function createKeyRange, Function getSplitMinRatio, Function getSplitMaxRatio) {
		this.getContainer = getContainer;
		this.determineContainer = determineContainer;
		initialize(getKey, keyConverter, dataConverter, createSeparator, createKeyRange, getSplitMinRatio, getSplitMaxRatio);
	...
	public Container container() {
		/**
		 * TODO: this doesn't seem to make much sense, as the signature of
		 * getContainer should be :: Tree.IndexEntry -> Container in
		 * {@link Tree}. Probably this only works here as there is never
		 * actually a real getter Function used, only Constants mapping on the
		 * given Container.
		 */
		return (Container) this.getContainer.invoke(this);
	}
	...
	/**
	 * Calls {@link #remove(Descriptor, int, Predicate)} whereas the target
	 * level is set to zero. The used <tt>Descriptor</tt> is the result of
	 * 
	 * <pre>
	 * <code>
	 ...
	 *            object to remove
	 * @param equals
	 *            predicate determining if an object in the tree equals
	 *            <tt>data</tt> and should therefore be removed
	...
	public Object remove(final Object data, final Predicate equals) {
		return remove((Descriptor) getDescriptor.invoke(data), 0, new AbstractPredicate() {
	 ...
	 * the given <tt>indexEntry</tt>. The path nodes are fixed in the underlying
	 * buffer. If they are no longer required the caller has to unfix them. If
	 * the given node is not a leaf node an
	 ...
	 * nodes are fixed in the underlying buffer. If they are no longer required
	 * the caller has to unfix them.
	 * 
	 * @param leaf
	 ...
	 * <tt>Node</tt> is specified by its key range. The level parameter is
	 * required to determine the target level. The path nodes are fixed in the
	 * underlying buffer. If they are no longer required the caller has to unfix
	 * them.
	 * 
	 * @param nodeRegion
	...
	public Cursor aloneKeyQuery(Comparable key) {
		if (!this.duplicate) {
			throw new UnsupportedOperationException("B+Tree does not support duplicates!!! Please run " + "exactMatchQuery(Comparable key) method ");
		}
		KeyRange range = createKeyRange(key, key);
		return query(range, 0);
	}
	 ...
	 *            case it has to an instance of <tt>Separator</tt> or
	 *            <tt>KeyRange</tt>
	 * @param targetLevel
	 ...
	 * the entries of the non-leaf nodes). Each <tt>IndexEntry</tt> refers to a
	 * {@link BPlusTree.Node Node} which is the root node of the subtree. Each
	 ...
	 * <tt>Containers</tt>. NOTE: if the tree is running in non-duplicate mode,
	 * the indexEntry descriptor holds the value of the left most element. In
	 * duplicate mode it is the right most
	 */
	public class Node extends Tree.Node {
		 ...
		 * <tt>Node</tt> more flexible, because it can use any implementation of
		 * the abstract interface {@link java.util.List}.
		 */
		...
		protected int rightMostSearch(Comparable key) {
			...
			while (index != (sepValues.size() - 1) && sepValues.get(index + 1).equals(duplicateKey))
				index++;
		...
		/**
		 * Treats overflows which occur during an insertion. It splits the
		 * overflowing node in new nodes. It creates new index-entries for the
		 * new nodes. These index-entries will be be added to the given
		 * <tt>List</tt>. If the Leaf Node is found to be full,
		 * {@link BPlusTree.Node#redistributeNode(Stack)} is attempted to avoid
		 * a split. The method
		 * {@link BPlusTree.Node#post(Tree.Node.SplitInfo, Tree.IndexEntry)}
		 * will later be used to post the created index-entries to the parent
		 * node.
		 * 
		 * @param path
		 *            the path from the root to the overflowing node
		 * @param newIndexEntries
		 *            a <tt>List</tt> to carry the new index-entries created by
		 *            the split
		 * @param up
		 *            signals whether the top node on stack will be written into
		 *            the storage by calling {@link Tree#update(Stack path)} and
		 *            unfixed from the underlying buffer by calling
		 *            {@link Tree#up(Stack path)}.
		 * @return a Collection which accrues from the given collection and the
		 *         created index-entries during the split.
		 */
		...
		/**
		 * This method searches the right sibling node for a possible insertion
		 * point, will be successful when right sibling is not full. The method
		 * is reserved for improved BPlusTree. In this implementation this
		...
		protected Tree.Node.SplitInfo split(Stack path) {
			...
			if (this.level() == 0 && duplicate) {
				...
				if (pivotEntry.compareTo(separator(node.entries.get((number / 4) * 3)).sepValue()) == 0) { //  75 %
					dupIndex = (number / 4) * 3;
					while (dupIndex > (number / 4) && pivotEntry.compareTo(separator(node.entries.get(dupIndex - 1)).sepValue()) == 0) {
						dupIndex--;
					}
					index = dupIndex;
				}
		...
		protected IndexEntry remove(IndexEntry indexEntry) {
			...
			int index = 0;
			if (duplicate) { // 
				while (!getEntry(index).equals(indexEntry) && index < this.number()) {
					index++;
				}
			} else {
		 ...
		 * @param path
		 *            the path from the root to parent of the <tt>Node</tt>
		...
		public class MergeInfo {
			 ...
			 * <ul>
			 * <li>It is the right sibling of the merged node &rarr; a copy of
			 * the <tt>Separator</tt> of the first entry of the sibling
			 * <tt>Node</tt></li>
	...
	public static abstract class KeyRange extends Separator {
		 ...
		 * @return <tt>true</tt> if the given <tt>Descriptor</tt> an instance of
		 *         of <tt>KeyRange</tt> and overlaps this <tt>KeyRange</tt> and
		 *         <tt>false</tt> otherwise.
		 * 
		...
		/**
		 * Checks whether the given <tt>Descriptor</tt> is totally contained in
		 * this <tt>KeyRange</tt>.
		 * 
		 * @param descriptor
		 *            the <tt>KeyRange</tt> to check
		 * @return <tt>true</tt> if the given <tt>Descriptor</tt> an instance of
		 *         <tt>KeyRange</tt> and lies totally in this <tt>KeyRange</tt>
		...
		protected boolean contains(Comparable key) {
			return minBound().compareTo(key) <= 0 && (maxBound == null || maxBound().compareTo(key) >= 0);
		}
		 ...
		 * contains the given key. The current <tt>KeyRange</tt> will be changed
		 * and returned.
		 * 
		 * @param key
		...
		public void union(Comparable key) {
			if (minBound().compareTo(key) > 0)
				updateMinBound(key);
			if (maxBound().compareTo(key) < 0)
				updateMaxBound(key);
		}
	...
	protected class QueryCursor extends xxl.core.indexStructures.QueryCursor {
		...
		protected void removeObject() {
			...
			if (rootDescriptor != null && rootEntry != null) {
				...
				if (counterRightShiftDup > 0)
					counterRightShiftDup--; // count down returned duplicate Objects
			} else {
		...
		protected void backToFirstEntry(Stack path, Separator prevSep, boolean computeIndex) {
			KeyRange prev = createKeyRange(prevSep.sepValue(), prevSep.sepValue());
			if (path == null) {
				path = new Stack();
			}
			if (path.isEmpty()) {
			...
			if (computeIndex) {
				...
				for (int i = 0, j = 1; i < currentNode.number(); i++) {
					index = i;
					if (separator(((Node) currentNode).getEntry(i)).isRightOf((prev).minBound())) {
						j++;
		...
		protected void updateObject(Object newData) {
			...
			if (!key(oldDat).equals(key(newData)))
				throw new IllegalArgumentException("The updated new data have to be the different key like the old one.");

			Object oldData = lastNode.remove(lastIndex);
			lastNode.grow(newData);
			lastIndexEntry.container().update(lastIndexEntry.id(), lastNode, true);
		}
	...
	/**
	 * A <tt>NodeConverter</tt> is used by the <tt>BPlusTree</tt> to convert the
	 * <tt>Nodes</tt> for I/O-purposes.
	 */
